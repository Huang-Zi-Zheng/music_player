...
window = tk.Tk()
window.title("Music_Player_v0.1")
window.geometry("400x600")
green =  (93, 135, 54)

# Create a label for the music player title
l_music_player = tk.Label(window, text="Music Player", font=("Helvetica", 30, "bold"))
l_music_player.pack(pady=9)
...
btn_select_folder = ctk.CTkButton(window, text="Select Music Folder",
                                  ...
                                  font=("Helvetica", 20), )
btn_select_folder.pack(pady=15)

# Create a listbox to display the available songs
lbox = tk.Listbox(window, width=30, font=("Helvetica", 20))
lbox.pack(pady=15)
...
btn_previous = ctk.CTkButton(btn_frame, text=" ⏮️", command=previous_song,
                             width=50, font=("Helvetica", 18))
btn_previous.pack(side=tk.LEFT, padx=5)

# Create a button to play the music
play_button = ctk.CTkButton(btn_frame, text=" ▶️ ", command=play_music, width=50,
                         font=("Helvetica", 20))
...
btn_next = ctk.CTkButton(btn_frame, text=" ⏭️", command=next_song, width=50,
                         font=("Helvetica", 18))
btn_next.pack(side=tk.LEFT, padx=5)

# Create a progress bar to indicate the current song's progress
pbar = Progressbar(window, length=200, mode="determinate")
pbar.pack(pady=10)
